...
ions_data = 
  ...
  select_if(~!(all(is.na(.)) | all(. == ""))) %>%
 mutate(nitrate_n = nitrate_ppm * (14/62))
...
```{r ions-maps}
make_map(DAT = ions_data %>% dplyr::select(kit_id, nitrate_n) %>% filter(nitrate_n <= 10) %>% filter(nitrate_n >= 0), VAR = "nitrate_n", COLOR_LEGEND = "Nitrate-N, ppm") + ggtitle("Surface waters: nitrate")
