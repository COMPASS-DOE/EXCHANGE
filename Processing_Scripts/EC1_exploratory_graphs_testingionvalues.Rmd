...
# import steph's combined spreadsheet for workshop

combined_data = googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1f6nvuvWGHsmb-vOynBUkIgnwAAymbEiXmj92UNUOaFA/edit#gid=1145965819", sheet = 1)

```

```{r by non-diluted ions}
# import readme file

nondil_ions = googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1iTdxdpoZBnzTuegJOkffav2wckO1KauILDd_-9SoDf8/", sheet = 1)

ionsdata.1 = all_data_processed %>% filter(transect_location == 'water') %>% filter(analysis == "Ions")

ionsdata = left_join(nondil_ions, ionsdata.1, by='kit_id')

```


```{r figuring out ion reruns}
# import readme file

dil_ions = googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1iTdxdpoZBnzTuegJOkffav2wckO1KauILDd_-9SoDf8/", sheet = 2)

need_2_rerun = anti_join(dil_ions, nondil_ions, by ='kit_id')

write.csv(need_2_rerun, file = "/Users/myer056/OneDrive - PNNL/Documents/GitHub/EXCHANGE/Data/ions/EC1_ions_rerun.csv")
...
```{r tctn, fig.width=8, fig.height=4}
tctn_data %>% 
...
```{r pH_graphs, fig.width=8, fig.height=4}
pH_data %>% 
...
```{r gwc, fig.width=8, fig.height=4}
gwc_data %>% 
...
```{r loi, fig.width=8, fig.height=4}
loi_data %>% 
...
ions_data = 
  ...
  select_if(~!(all(is.na(.)) | all(. == ""))) %>%
 mutate(nitrate_n_ppm = nitrate_ppm * (14/62)) %>%
 mutate(nitrite_n_ppm = nitrite_ppm * (14/46))

ions_data_long = 
  ions_data %>% 
  pivot_longer(ends_with("_ppm")) %>% 
  filter(!is.na(value))
```

```{r ions, fig.width=12, fig.height=8}
ions_data_long %>% 
...
```{r ions-maps}
make_map(DAT = ions_data %>% dplyr::select(kit_id, nitrate_n_ppm), VAR = "nitrate_n_ppm", COLOR_LEGEND = "Nitrate-N, ppm") + ggtitle("Surface waters: nitrate")
make_map(DAT = ions_data %>% dplyr::select(kit_id, nitrite_n_ppm), VAR = "nitrite_n_ppm", COLOR_LEGEND = "Nitrite-N, ppm") + ggtitle("Surface waters: nitrite")
...
make_map(DAT = ions_data %>% dplyr::select(kit_id, sulfate_ppm), VAR = "sulfate_ppm", COLOR_LEGEND = "Sulfate, ppm") + ggtitle("Surface waters: sulfate")
make_map(DAT = ions_data %>% dplyr::select(kit_id, phosphate_ppm), VAR = "phosphate_ppm", COLOR_LEGEND = "Phosphate, ppm") + ggtitle("Surface waters: phosphate")
```
...
```{r cdom, fig.width=9, fig.height=6}
cdom_data_long %>% 
  ggplot(aes(x = region, y = value))+
...
```{r doc, fig.width=6, fig.height=4}
npoc_data %>% 
...
```{r tss, fig.width=6, fig.height=4}
tss_data %>% 
...
```{r waterq, fig.width=12, fig.height=4}
waterquality_data_long %>% 
  ggplot(aes(x = region, y = value))+
  geom_jitter(width = 0.1)+
