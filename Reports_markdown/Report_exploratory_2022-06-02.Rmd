...
make_map = function(DAT, VAR, COLOR_LEGEND){
  ...
  base_plot <- ggplot() + 
    ...
    geom_sf(data = df_map, color = "white",
            size = point_size * 2.5) + 
    geom_sf(data = df_map, aes_string(color = VAR), 
            size = point_size * 2) + 
  ...
  inset_plot <- ggplot() + geom_sf(data = cb_states) + 
    ...
    geom_sf(data = df_cb, aes_string(color = VAR), size = point_size * 1.7) + 
    theme_map() + 
    theme(legend.background = element_rect(fill = alpha("white", 0.0)), 
          legend.key = element_rect(fill = "transparent"), 
...
```{r}
make_map(DAT = ions_data %>% dplyr::select(kit_id, calcium_ppm), VAR = "calcium_ppm", COLOR_LEGEND = "calcium_ppm")
make_map(DAT = ions_data %>% dplyr::select(kit_id, chloride_ppm), VAR = "chloride_ppm", COLOR_LEGEND = "chloride_ppm")
make_map(DAT = ions_data %>% dplyr::select(kit_id, sulfate_ppm), VAR = "sulfate_ppm", COLOR_LEGEND = "sulfate_ppm")
make_map(DAT = ions_data %>% dplyr::select(kit_id, nitrite_ppm), VAR = "nitrite_ppm", COLOR_LEGEND = "nitrite_ppm")
```
...
```{r}
make_map(DAT = cdom_data %>% dplyr::select(kit_id, SUVA254), VAR = "SUVA254", COLOR_LEGEND = "SUVA254")
...
make_map(DAT = cdom_data %>% dplyr::select(kit_id, FI), VAR = "FI", COLOR_LEGEND = "FI")
...
```{r npoc_map}
make_map(DAT = npoc_data %>% dplyr::select(kit_id, npoc_mgl), VAR = "npoc_mgl", COLOR_LEGEND = "NPOC, mg/L")
```
...
```{r tss_map}
make_map(DAT = tss_data %>% dplyr::select(kit_id, tss_mg_perl), VAR = "tss_mg_perl", COLOR_LEGEND = "TSS, mg/L")
```
...
make_map(DAT = waterquality_data %>% dplyr::select(kit_id, sal_psu), VAR = "sal_psu", COLOR_LEGEND = "sal_psu") + ggtitle("Surface waters: salinity")
```
